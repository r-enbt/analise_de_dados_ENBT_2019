...
"CRAN é uma rede de servidores ftp e web servers ao redor do mundo que guarda versões identicas, atualizadas, do código e documentação para R. Por favor use o espelho (_mirror_) de CRAN mais próximo de você para minimizar a carga da rede." 
...
#### A recomendação é depender do [__script__]() e salvar os  objetos maiores no HD (para que estes possam ser lidos).
...
+ Também é a unidade de avaliação de cursos e a única maneira de receber ajuda de alguém... 
...
* Checar a pasta atual de trabalho com `getwd()` e `pwd` no terminal
